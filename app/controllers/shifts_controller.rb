class ShiftsController < ApplicationController
  ...
  def index
    ...
    if params['filter']
      ...
      if can_select == false
        @shifts = Shift.from_today(from_today).by_shift_type(sts).by_date(dt).by_day_of_week(dow).by_users(usrs).by_holidays(holidays).by_unselected(true).paginate(:page => params[:page], :per_page => 150)
      else
