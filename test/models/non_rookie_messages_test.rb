...
class NonRookieMessageTest < ActiveSupport::TestCase
  ...
  def test_round_two_status_messages
    ...
    puts HostUtility.bingo_start_for_round(@group3_user, 2)
    puts "Date: #{Date.today}"

    puts @group3_user.shift_status_message
    ...
    Shift.all.each do |s|
      if s.can_select(@group3_user, HostUtility.can_select_params_for(@group3_user))
        ...
        if num < 12 then
          ...
          @group3_user.shift_status_message.include?("All required shifts selected for round 1. (12 of 12)").must_equal true
        end
      end
    end
