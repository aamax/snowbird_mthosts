...
class RookieMessageTest < ActiveSupport::TestCase
  ...
  def test_T1_training_shifts_selected
    @sys_config.bingo_start_date = HostUtility.bingo_start_for_round(@rookie_user, 0)
    @sys_config.save
    create_t1_shifts
    # create_t2andt3_shifts
    ...
    msgs.include?("You are currently in <strong>round 0</strong>.").must_equal true
    num = 0
    Shift.where("short_name = 'T1'").each do |s|
      break if num >= 4
  ...
  def test_round_three_status_messages
    ...
    msgs.include?("All required shifts selected for round 3. (20 of 20)").must_equal true
  end
  ...
  def test_round_four_status_messages
    ...
    Shift.all.each do |s|
      ...
      if @rookie_user.shifts.count < 20
        msgs = @rookie_user.shift_status_message
        msgs.include?("#{shift_count} of #{20} Shifts Selected.  You need to pick #{20 - shift_count}").must_equal true
      end
    end
  ...
  def test_after_bingo_messages
    ...
    FactoryBot.create(:shift, shift_date: last_date, shift_type_id: @p1.id)
